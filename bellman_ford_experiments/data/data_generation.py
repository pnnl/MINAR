...
def path_graph(K, A, m = None, rng=None):
    ...
    edge_index, edge_attr = pyg.utils.add_self_loops(edge_index, edge_attr, fill_value=0, num_nodes=K+1)
...
def _nx_to_test_data(G, K, rng=None):
    ...
    data.edge_index, data.edge_attr = pyg.utils.add_self_loops(data.edge_index, data.edge_attr, fill_value=0, num_nodes=data.num_nodes)
    return data
